...
def main(data_path, output_file, train_percent=0.1, batch_size=320, epochs=150, gpu=0, seeds=[42]):
    ...
    for idx, seed in enumerate(seeds):
        ...
        train_loader, val_loader, test_loader = datasets.get_CIFAR10(data_path, batch_size)

        model = ResNet18()
        trainer = Trainer(model, val_loader, test_loader, selection=uniform_selection, train_percent=train_percent,
                          device=device)
        ...
        for epoch in tqdm(range(epochs), position=0, leave=True, colour='red', desc=prefix):
        ...
        filename = f"baseline_CIFAR10_Resnet18_{seed}.pt"
...
if __name__ == '__main__':
    ...
    main(data_path=CONFIG['data_path'], output_file=os.path.join(CONFIG['out_path'], 'cifar10_baseline.pickle'), batch_size=CONFIG['batch_size'],
